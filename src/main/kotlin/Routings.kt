...
const val S3_UPLOADS_PER_SECOND = 400
const val S3_UPLOAD_CONCURRENCY = 32
...
private suspend fun copyMapsToS3() {
    ...
    try {
        ...
        coroutineScope {
            ...
            val rateLimiter = Channel<Unit>(S3_UPLOADS_PER_SECOND)
            val concurrencyLimiter = Semaphore(S3_UPLOAD_CONCURRENCY)

            val rateJob = launch {
                while (isActive) {
                    repeat(S3_UPLOADS_PER_SECOND) {
                        rateLimiter.send(Unit)
                    }
                    delay(1.seconds)
                }
            }
